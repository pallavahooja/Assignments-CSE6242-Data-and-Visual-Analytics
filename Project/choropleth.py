...
def get_dataframe():
    df = pd.read_csv('final_op_sentiments.csv')
...
app.layout = html.Div([
    ...
    html.Div(dcc.Slider(
        ...
        marks={date: datetime.datetime.fromtimestamp(date).strftime('%m%d%Y') for date in df['timestamp'].unique()},
        step=None
...
def update_graph(date):
    ...
    print(dff.head())
    print(counties)
    fig = px.choropleth(dff, locationmode = "ISO-3", locations = 'Country Name', color = 'Average Sentiment', color_continuous_scale="Viridis")
...
def create_time_series(dff, text):
    ...
    fig = px.scatter(dff, x='Date', y='Average Sentiment')
