...
def get_dataframe():
    df = pd.read_csv('final_op_sentiments_weekly.csv')
...
app.layout = html.Div([
    ...
    html.Div(dcc.Slider(
        ...
        marks={int(date): datetime.datetime.fromtimestamp(date).strftime('%m/%d') if i%2 == 0 else "" for i, date in enumerate(df['timestamp'].unique())},
        step = None
